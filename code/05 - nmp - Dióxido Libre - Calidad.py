...
i = 5 # columna que queremos obtener
x = np.array([mnp[:,i]]) # Extrae la columna i y la guarda en x
print('Variable Independiente X: Dióxido Libre')
print(x)
print(x.shape)

print('Variable Independiente X Transpuesta: Dióxido Libre')
x = np.transpose(x)
print(x)
print(x.shape)
...
print('Variable Independiente X + BIAS: BIAS Dióxido Libre')
print(x_b)
print(x_b.shape)
...
#plt.show()
plt.savefig('plots\DióxidoLibre_vs_calidad_numpy.png')
#"""
...
plt.title('Regresión Lineal con Numpy')
#plt.show()
plt.savefig('plots\y_vs_y_pred_dlq_numpy.png')
#"""
