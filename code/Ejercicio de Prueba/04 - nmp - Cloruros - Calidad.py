...
i = 4 # columna que queremos obtener
x = np.array([mnp[:,i]]) # Extrae la columna i y la guarda en x
print('Variable Independiente X: Cloruros')
print(x)
print(x.shape)

print('Variable Independiente X Transpuesta: Cloruros')
x = np.transpose(x)
print(x)
print(x.shape)
...
print('Variable Independiente X + BIAS: BIAS Cloruros')
print(x_b)
print(x_b.shape)
...
plt.show()
plt.savefig('plots\Cloruros_vs_calidad_numpy.png')
...
plt.title('Regresión Lineal con Numpy')
plt.show()
plt.savefig('plots\y_vs_y_pred_clq_numpy.png')
#"""
